...
export default function AdminChatPage() {
  ...
  const loadMessages = useCallback(async () => {
    try {
      ...
      // Backend yangidan-eskiga qaytaradi; chat tartibida eskidan-yangiga ko'rsatish uchun reverse
  ...
  // Auto-scroll to bottom when messages tab is open and new messages arrive
  useEffect(() => {
    ...
    <div className="space-y-4">
      ...
      {tab === "messages" && (
        <div className="border border-border rounded-2xl bg-card overflow-hidden flex flex-col h-[70vh]">
          {/* Chat header */}
          <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border bg-accent/40 text-xs text-muted-foreground">
            <MessageCircle className="w-3.5 h-3.5" />
            Umumiy chat
            <span className="text-muted-foreground/60">·</span>
            <span>{messages.length} ta xabar</span>
          </div>
              ...
              messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    {!isMe && (
                      msg.photo_url ? (
                        <img src={msg.photo_url} alt="" className="w-8 h-8 rounded-full object-cover shrink-0" />
                      ) : (
                    ...
                    <div className={`flex flex-col max-w-[70%] ${isMe ? "items-end" : "items-start"}`}>
                      ...
                      <div className={`flex items-end gap-1.5 ${isMe ? "flex-row-reverse" : ""}`}>
                        ...
                        >
                          {isMe && <div className="text-[10px] font-semibold text-indigo-200/80 mb-0.5">Admin</div>}
                          <p className="whitespace-pre-wrap leading-snug">{msg.content}</p>
                          <span className={`block text-[10px] mt-1 text-right ${isMe ? "text-indigo-100/70" : "text-muted-foreground"}`}>
                        ...
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1">
                          ...
                          {!isMe && (
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-background/80 border border-border hover:bg-orange-500/10 hover:text-orange-400 transition-colors"
                              title="Bloklash"
                            >
          ...
          <div className="flex items-center gap-2 px-3 py-3 border-t border-border bg-background/60 backdrop-blur">
            <input
              ...
              placeholder={replyingTo ? `${replyingTo.username} ga javob yozing...` : "Xabar yozing..."}
              className="flex-1 bg-muted border border-border rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
              disabled={sending}
            />
            <button
              onClick={handleSendMessage}
              disabled={sending || !newMessage.trim()}
              className="p-2.5 bg-indigo-600 text-white rounded-full hover:bg-indigo-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
            >
